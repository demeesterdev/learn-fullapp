    ...
    "mutations": {
        ADD_POST(state, post) {
            if (state.posts.length < 1) {
              ...
              const max = state.posts.reduce((prev,current) => prev.id > current.id ? prev : current);
              post.id = max.id + 1;
            }
            post.comments = [];
        ...
        ADD_COMMENT(state,{postId, comment}) {
          ...
          if (post.comments.length < 1) {
            ...
            const max = post.comments.reduce((prev,current) => prev.id > current.id ? prev : current)
            comment.id = max.id + 1;
          }
